---
layout: home

hero:
  name: qsToolBox
  text: アバター改変効率化ツール
  tagline: VRChat アバター改変作業をもっとスムーズに
  actions:
    - theme: brand
      text: インストール
      link: /install
    - theme: alt
      text: 機能一覧
      link: /features/contextmenu

features:
  - title: 体の明るさが場所によってバラバラ、角度によってパーツまで消えてしまう
    details: アバターのルートに必要なコンポーネントを一括追加することで改善できます。
    link: /features/contextmenu#アバタールート用コンポーネント追加
    linkText: 使い方を見る
  - title: フォルダ移動のたびに何度もクリックして戻っている
    details: よく使うフォルダをブックマークしておくと、ボタン一つで素早く移動できます。
    link: /features/bookmark
    linkText: 使い方を見る
  - title: 作業中、オブジェクトをサクッと隠したり EditorOnly にしたい
    details: 選択した状態で Ctrl + E を押すだけで、表示・非表示と EditorOnly をまとめて切り替えられます。
    link: /features/editoronly
    linkText: 使い方を見る
  - title: 色違いマテリアル、切り替えるのが毎回面倒！
    details: 衣装や髪を選ぶだけでマテリアルが一覧表示され、ドラッグ＆ドロップで差し替えられます。
    link: /features/material
    linkText: 使い方を見る
  - title: 衣装のスケール、素体に合わせて一括で直したい
    details: 衣装を選ぶと、素体のスケールに一発で同期できます。Scale Adjuster にも対応！
    link: /features/scale
    linkText: 使い方を見る
  - title: 顔改変後、まばたきが干渉してしまっている
    details: まばたきをベースに選び、改変した目の形をマイナス値で追加して合成するだけで直せます。
    link: /features/blendshape#使用例
    linkText: 使い方を見る
---

## 具体的に何が変わるの？

- **Ctrl + E** — 選んだオブジェクトをすぐ EditorOnly / 非表示に切り替え。作業中に邪魔なパーツをサッと隠せます
- **Ctrl + Q** — 選択中の衣装やオブジェクトに使えるツールをまとめて呼び出し。マテリアル差し替え・スケール同期・ブレンドシェイプ編集などが一か所に
- **右クリックメニュー** や **シーンビュー上部のバー** にも機能を追加。よく使うフォルダへのブックマークや、ルートへのコンポーネント一括追加もできます
